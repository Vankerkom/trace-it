...
4. **`src/trace-api.ts`** — thin wrapper around the trace.moe HTTP API: `queryAccount` (quota/`GET /me`), `search` (`POST /search` with all of a file's extracted frame vectors batched into one request, optionally scoped to an AniList ID; throws `QuotaExceededError` with the real `quota`/`quotaUsed` on a `402`), `checkUserQuota`.
5. Back in **`src/run.ts`**:
   - `identifyAnimeEpisode` sends all extracted frame hashes for a file in a single batched `search` call (via `searchWithQuotaFallback`), filters matches below `SIMILARITY_THRESHOLD` (0.885), buckets remaining matches by `seriesId:episode`, and requires `MIN_VOTES_REQUIRED` (4) matching frames before picking a winner (ranked by average similarity, then vote count, then best single similarity). This multi-frame voting is what makes the tool robust against a single frame matching a recap/OP/ED.
   - `getActiveKey` / `getAccount` / `updateAccountQuota` implement quota management: try the public/free trace.moe tier first, fall back to `TRACE_API_KEY` from config only if the free tier is exhausted; account info is cached in-memory for `ACCOUNT_CACHE_TTL` (1 minute), and each `/search` response's real `quota`/`quotaUsed` fields are written back into the cache to avoid re-querying `/me` before every search. `searchWithQuotaFallback` never calls `search` for a tier it doesn't already believe has enough budget for `FRAME_EXTRACT_COUNT`, but if a live `402` proves that cached belief stale (e.g. the shared public/IP tier got drained by other users), it marks that tier exhausted immediately and falls back to `TRACE_API_KEY` if one is configured. Once nothing is left to try, `reportQuotaExhausted` prints a donation nudge (no key configured) or a guessed ~24h retry time (key also exhausted), and `QuotaExhaustedFatalError` stops the whole run.
